...
// ═══ DEAL CHECK TAB SET ═══
// The five the strip was missing. Everything else it needs was already here — IconBox, IconFlower,
// IconFactory, IconCart, IconBolt — which is the reason these are drawn to the same 24×24, 1.75
// stroke, currentColor rules rather than to whatever each one looked best at on its own. A tab strip
// is read as a ROW, and one icon with a different weight is the one you keep noticing.

// Truss — a lattice span, which is what the department actually deals in.
export function IconTruss({ size = 15 }) {
  return (
    <svg {...svg(size)}>
      <path d="M2.5 7.5h19M2.5 16.5h19" />
      <path d="M2.5 7.5 7 16.5M7 7.5l4.5 9M11.5 7.5l4.5 9M16 7.5l4.5 9" />
    </svg>
  );
}

// Manpower — a crew, not a person: the tab is about how many hands, not who.
export function IconCrew({ size = 15 }) {
  return (
    <svg {...svg(size)}>
      <circle cx="9" cy="8" r="3.1" />
      <path d="M3.5 20v-1.2A4.3 4.3 0 0 1 7.8 14.5h2.4a4.3 4.3 0 0 1 4.3 4.3V20" />
      <path d="M16.2 6.2a3 3 0 0 1 0 5.6M18 20v-1.4a4 4 0 0 0-2.3-3.6" />
    </svg>
  );
}

// Transport — a truck in profile. The load box leads, because that is what is being costed.
export function IconTruck({ size = 15 }) {
  return (
    <svg {...svg(size)}>
...
// Inventory status — bars, because the tab answers "how much of each, against what we hold".
export function IconChart({ size = 15 }) {
  return (
    <svg {...svg(size)}>
      <path d="M3.5 20.5V3.5" />
      <path d="M3.5 20.5h17" />
      <path d="M8 20.5v-6M13 20.5v-10M18 20.5v-4" />
    </svg>
  );
}

// GYV & buffer — stacked coins. Percentages taken off the top, which is what a stack reads as.
export function IconCoins({ size = 15 }) {
  return (
    <svg {...svg(size)}>
