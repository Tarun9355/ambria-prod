...
export default function StudioApp() {
  ...
  const reconcileSoldInventoryBlocks = useCallback(async (fnsToProcess, newCards, inventoryList, cli) => {
    ...
    // Always resync functions.items, regardless of whether anything changed vs. our OWN diff cache
    // (dcReservedInventory) below. That cache only tracks what THIS incremental path itself last
    // wrote — it says nothing about whether functions.items (a completely separate table, synced
    // only once at markSold by createProjectFromEO) actually matches reality. Gating this on
    // `changed` meant a rerun with no new edits — e.g. just reopening Deal Check to retest — skipped
    // this sync entirely, silently, even when functions.items had never been correct in the first
    // place. Self-heals every run instead; the redundant no-op writes on an already-correct state
  ...
  const ctx = {
    ...
    getLibPhotosForZone, maxRepaintCostInSubcat, saveSession, markSold, unbookDeal, loadEvent,
