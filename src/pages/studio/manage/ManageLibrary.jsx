                      ...
                      return (
                        <Fragment key={idx}>
                          <div style={{ fontSize: 11, fontWeight: 500, color: invItem ? textP : "#F59E0B", display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                            ...
                            {isFloralBlend && <span style={{ display: "flex", alignItems: "center", gap: 2, fontSize: 9, color: "#EC4899", fontWeight: 600 }}>🌸<input type="number" min="0" max="100" value={realPct} onChange={e => { const v = Math.max(0, Math.min(100, Number(e.target.value) || 0)); const elems = [...(libEditImg.elements || [])]; elems[idx] = { ...elems[idx], realPct: v }; setLibEditImg({ ...libEditImg, elements: elems }); }} style={{ width: 28, padding: "1px 3px", borderRadius: 3, border: `1px solid ${border}`, fontSize: 9, fontWeight: 700, textAlign: "center" }} />%{realPct !== 100 && <button onClick={() => { const elems = [...(libEditImg.elements || [])]; elems[idx] = { ...elems[idx], realPct: 100 }; setLibEditImg({ ...libEditImg, elements: elems }); }} title="Price this element at 100% the recipe's Studio rate, overriding the sub-category's default" style={{ padding: "1px 5px", borderRadius: 3, border: "none", fontSize: 8, fontWeight: 700, cursor: "pointer", background: "rgba(236,72,153,0.12)", color: "#EC4899" }}>💯 100%</button>}</span>}
                          </div>
