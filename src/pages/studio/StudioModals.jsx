...
export default function StudioModals({ ctx }) {
  const {
    ...
    // zoneUploadReview
    zoneUploadReview, setZoneUploadReview, closeZoneUploadReview, zoneLabelsD, accent, cardBg, S,
  ...
  return (<>
                      ...
                      <div key={it.id} onClick={()=>inSplit ? toggleSplitId(it.id) : setAvailModal(m=>({...m,selectedId: sel?null:it.id}))}
                        ...
                        <div style={{padding:"8px 10px"}}>
                          ...
                          {/* The corner badge is deliberately a bare number (no room for a unit in a
                              22px-wide tag) — this line spells it out. availModal.unitLabel is a
                              caller override (Deal Check's carpet picker passes "sqft") for when the
                              raw inventory unit field doesn't say what this qty actually means here —
                              a carpet's own `unit` is often a generic "Piece" (how it's counted on
                              the shelf) even though every carpet cost calc in this app (calcZoneCarpet,
                              the split editor's own "sqft avail") treats its qty as total sqft
      ...
      {zurCustomPicker && (
        <InventoryItemPickerModal
          title={zurCustomPicker.kind === "ceiling" ? "Custom Ceiling — Fabric › Ceiling" : "Custom Masking — Fabric › Printed Walls"}
          icon={zurCustomPicker.kind === "ceiling" ? "🎬" : "🖼️"}
          accent="#7C3AED"
          imsInventory={imsInventory}
          categoryMatch="fabric"
          subcatMatch={zurCustomPicker.kind === "ceiling" ? "ceiling" : "printed wall"}
          rcFactorByKey={rcFactorByKey}
          onSelect={(item) => {
            const field = zurCustomPicker.kind === "ceiling" ? "customCeilingItemId" : "customMaskingItemId";
            if (zurCustomPicker.ri == null) {
              setZoneUploadReview({ ...zoneUploadReview, dims: { ...(zoneUploadReview.dims || {}), [field]: item.id } });
            } else {
              const rows = [...((zoneUploadReview.dims || {}).trussRows || [])];
              rows[zurCustomPicker.ri] = { ...rows[zurCustomPicker.ri], [field]: item.id };
              setZoneUploadReview({ ...zoneUploadReview, dims: { ...(zoneUploadReview.dims || {}), trussRows: rows } });
            }
            setZurCustomPicker(null);
          }}
          onClose={() => setZurCustomPicker(null)}
          isDark={isDark} border={border} textP={textP} textS={textS} cardBg={cardBg}
        />
      )}
      {/* Zone Upload Review Modal */}
      {zoneUploadReview&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>closeZoneUploadReview()}>
        <div style={{background:cardBg,borderRadius:16,maxWidth:700,width:"100%",maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
          <div style={{padding:"16px 20px",borderBottom:`1px solid ${border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:accent+"08"}}>
            ...
            <button onClick={()=>closeZoneUploadReview()} style={{background:"transparent",border:"none",color:textS,fontSize:18,cursor:"pointer",fontWeight:700}}>✕</button>
          </div>
          <div style={{flex:1,overflowY:"auto",padding:"16px 20px"}}>
            ...
            <div style={{marginBottom:16,padding:12,background:isDark?"#0F0F1A":"#F9FAFB",borderRadius:10,border:`1px solid ${border}`}}>
                    ...
                    {(row.trussW || row.trussH) && (
                      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6, flexWrap:"wrap" }}>
                        ...
                        {rIsBox && (() => {
                          ...
                          return <button onClick={()=>setZurCustomPicker({ kind:"ceiling", ri })} style={{ padding:"2px 7px", borderRadius:5, fontSize:9, border:`1px dashed ${border}`, background:"transparent", color:textS, cursor:"pointer", marginLeft:4 }}>🎬 Custom Ceiling</button>;
                        })()}
                      </div>
                    )}
                    {rIsBox && (
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap", alignItems:"center" }}>
                          ...
                          {(() => {
                            ...
                            return <button onClick={()=>setZurCustomPicker({ kind:"masking", ri })} style={{ padding:"3px 8px", borderRadius:6, fontSize:9, border:`1px dashed ${border}`, background:"transparent", color:textS, cursor:"pointer" }}>🖼️ Custom Masking</button>;
                          })()}
                        </div>
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                ...
                <div><div style={{ fontSize: 9, color: textS, marginBottom: 2 }}>Floor Depth (ft)</div><input type="number" value={zoneUploadReview.dims?.floorL || ""} onChange={e => setZoneUploadReview({ ...zoneUploadReview, dims: { ...(zoneUploadReview.dims || {}), cpT: zoneUploadReview.dims?.cpT || defaultCarpetMatId(imsCarpetMaterials), floorL: parseFloat(e.target.value) || 0 } })} style={{ ...S.input, fontSize: 13, padding: "6px 8px", textAlign: "center", fontWeight: 600 }} placeholder="—" /></div>
                <div><div style={{ fontSize: 9, color: textS, marginBottom: 2 }}>Floor Width (ft)</div><input type="number" value={zoneUploadReview.dims?.floorW || ""} onChange={e => setZoneUploadReview({ ...zoneUploadReview, dims: { ...(zoneUploadReview.dims || {}), cpT: zoneUploadReview.dims?.cpT || defaultCarpetMatId(imsCarpetMaterials), floorW: parseFloat(e.target.value) || 0 } })} style={{ ...S.input, fontSize: 13, padding: "6px 8px", textAlign: "center", fontWeight: 600 }} placeholder="—" /></div>
          ...
          <div style={{padding:"14px 20px",borderTop:`1px solid ${border}`,display:"flex",gap:10,justifyContent:"flex-end"}}>
            <button onClick={()=>closeZoneUploadReview()} style={S.btn(false)}>Cancel</button>
