  ...
  return (
    <div className="space-y-4">
      ...
      {activePanel === "trussbatta" && trussInv && (() => {
        // Per-material truss inventory. Iron = the top-level trussInv (back-compat with all pricing
        // consumers). Pole/Aluminium = trussInv.materials[mat]. Batta stays SHARED (Iron tab only).
        const MAT_TABS = [{ k: "pole", l: "🪵 Pole" }, { k: "iron", l: "⛓️ Iron" }, { k: "aluminium", l: "⚙️ Aluminium" }];
        const isIron = trussMatTab === "iron";
        const TI = isIron ? trussInv : (trussInv.materials?.[trussMatTab] || {});
        const setTI = (updater) => {
          if (isIron) { setTrussInv(updater); return; }
          setTrussInv((ti) => ({ ...ti, materials: { ...(ti.materials || {}), [trussMatTab]: updater(ti.materials?.[trussMatTab] || {}) } }));
        };
        const matLabel = (MAT_TABS.find((m) => m.k === trussMatTab)?.l || "Iron").replace(/^\S+\s/, "");
        return (
        <div className="space-y-4">
          {/* ── Material tabs ── */}
          <div className="flex gap-2">
            {MAT_TABS.map((m) => (
              <button key={m.k} onClick={() => setTrussMatTab(m.k)} className={"px-5 py-2 rounded-xl text-sm font-semibold transition " + (trussMatTab === m.k ? "bg-indigo-600 text-white shadow" : "bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50")}>{m.l}</button>
            ))}
          </div>
          ...
          <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="font-bold text-indigo-900">{matLabel} Truss Inventory{isIron ? " · Batta · Liza" : ""}</p>
                <p className="text-xs text-indigo-700 mt-0.5">Skeleton (pillars + beams) per material. {isIron ? "Batta cloth + Liza fabric are shared across all materials and configured here." : "Batta + Liza are shared — configure them in the Iron tab."} Rates start at ₹0 — fill in below.</p>
              </div>
            </div>
            <div className={"grid gap-3 mt-4 " + (isIron ? "grid-cols-4" : "grid-cols-2")}>
              <div className="bg-white rounded-xl p-3 text-center"><div className="text-2xl font-bold text-indigo-700">{Object.values(TI.pillars || {}).reduce((s, p) => s + (p.stock || 0), 0)}</div><div className="text-xs text-gray-500">Pillar pieces</div></div>
              <div className="bg-white rounded-xl p-3 text-center"><div className="text-2xl font-bold text-indigo-700">{Object.values(TI.beams || {}).reduce((s, b) => s + (b.stock || 0), 0)}</div><div className="text-xs text-gray-500">Beam pieces</div></div>
              {isIron && <div className="bg-white rounded-xl p-3 text-center"><div className="text-2xl font-bold text-amber-700">{(trussInv.batta?.stockRft || 0).toLocaleString("en-IN")}</div><div className="text-xs text-gray-500">Batta RFT pool <span className="text-[9px] text-gray-400">(shared)</span></div></div>}
              {isIron && <div className="bg-white rounded-xl p-3 text-center"><div className="text-2xl font-bold text-rose-700">{(trussInv.liza?.stockKg || 0).toLocaleString("en-IN")}</div><div className="text-xs text-gray-500">Liza kg pool <span className="text-[9px] text-gray-400">(shared)</span></div></div>}
            </div>
          </div>
          ...
          <div className="bg-white border border-indigo-100 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div><p className="font-bold text-indigo-800">🏛️ {matLabel} Pillars (load-bearing tops)</p><p className="text-xs text-gray-500">Standard heights 10/12/15 ft. Single piece, 0 joints.</p></div>
              <button onClick={() => { const sz = prompt("New pillar size (ft, e.g. 8 or 18):"); if (!sz) return; const key = String(parseInt(sz) || 0); if (!key || key === "0") { alert("Invalid size"); return; } if (TI.pillars?.[key]) { alert("Size already exists"); return; } setTI((ti) => ({ ...ti, pillars: { ...ti.pillars, [key]: { stock: 0, name: `Pillar ${key}ft` } } })); }} className="text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg font-medium">+ Add Size</button>
            </div>
            {Object.keys(TI.pillars || {}).length === 0 ? (
              <div className="text-xs text-gray-500 italic border border-dashed border-indigo-200 rounded-xl p-4 text-center">No {matLabel} pillars yet — add sizes above. Until configured, {matLabel} trusses price using the Iron stock/rates.</div>
            ) : (
            <div className="overflow-hidden border border-indigo-100 rounded-xl">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {Object.entries(TI.pillars || {}).sort(([a], [b]) => parseInt(b) - parseInt(a)).map(([size, p]) => {
                    ...
                    return (
                      <tr key={size} className="border-t border-indigo-50">
                        ...
                        <td className="px-3 py-2"><input type="text" value={p.name || ""} onChange={(e) => setTI((ti) => ({ ...ti, pillars: { ...ti.pillars, [size]: { ...p, name: e.target.value } } }))} className="w-full border border-indigo-200 rounded px-2 py-1 text-xs" /></td>
                        <td className="px-3 py-2 text-center"><input type="number" min="0" value={p.stock || 0} onChange={(e) => setTI((ti) => ({ ...ti, pillars: { ...ti.pillars, [size]: { ...p, stock: parseInt(e.target.value) || 0 } } }))} className="w-20 border border-indigo-200 rounded px-2 py-1 text-sm text-center font-bold" /></td>
                        <td className="px-3 py-2 text-right text-gray-500 text-xs">{totalRft.toLocaleString("en-IN")} ft</td>
                        <td className="px-3 py-2 text-center"><button onClick={() => { if (!confirm(`Delete ${size}ft pillar?`)) return; setTI((ti) => { const next = { ...ti.pillars }; delete next[size]; return { ...ti, pillars: next }; }); }} className="text-red-400 hover:text-red-600 text-xs">✕</button></td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="mt-3 bg-indigo-50 border border-indigo-100 rounded-xl p-3 flex items-center justify-between">
              <div><p className="text-sm font-bold text-indigo-800">💰 {matLabel} pillar rental rate</p><p className="text-xs text-indigo-600">Single rate for all pillar sizes (₹/RFT × height).</p></div>
              <div className="flex items-center gap-2"><span className="text-sm text-gray-700">₹</span><input type="number" min="0" value={TI.rates?.pillarRftRate || 0} onChange={(e) => setTI((ti) => ({ ...ti, rates: { ...ti.rates, pillarRftRate: parseFloat(e.target.value) || 0 } }))} className="w-24 border border-indigo-300 rounded-lg px-3 py-1.5 text-sm font-bold text-indigo-700 text-center" /><span className="text-sm text-gray-700">/ RFT</span></div>
            </div>
          </div>
          ...
          <div className="bg-white border border-purple-100 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div><p className="font-bold text-purple-800">🔗 {matLabel} Beams (horizontal spans)</p><p className="text-xs text-gray-500">Sizes 2/3/4/5/8/10/12/15 ft.</p></div>
              <button onClick={() => { const sz = prompt("New beam size (ft, e.g. 6 or 20):"); if (!sz) return; const key = String(parseInt(sz) || 0); if (!key || key === "0") { alert("Invalid size"); return; } if (TI.beams?.[key]) { alert("Size already exists"); return; } setTI((ti) => ({ ...ti, beams: { ...ti.beams, [key]: { stock: 0, name: `Beam ${key}ft` } } })); }} className="text-xs bg-purple-600 hover:bg-purple-700 text-white px-3 py-1.5 rounded-lg font-medium">+ Add Size</button>
            </div>
            {Object.keys(TI.beams || {}).length === 0 ? (
              <div className="text-xs text-gray-500 italic border border-dashed border-purple-200 rounded-xl p-4 text-center">No {matLabel} beams yet — add sizes above.</div>
            ) : (
            <div className="overflow-hidden border border-purple-100 rounded-xl">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {Object.entries(TI.beams || {}).sort(([a], [b]) => parseInt(b) - parseInt(a)).map(([size, b]) => {
                    ...
                    return (
                      <tr key={size} className="border-t border-purple-50">
                        ...
                        <td className="px-3 py-2"><input type="text" value={b.name || ""} onChange={(e) => setTI((ti) => ({ ...ti, beams: { ...ti.beams, [size]: { ...b, name: e.target.value } } }))} className="w-full border border-purple-200 rounded px-2 py-1 text-xs" /></td>
                        <td className="px-3 py-2 text-center"><input type="number" min="0" value={b.stock || 0} onChange={(e) => setTI((ti) => ({ ...ti, beams: { ...ti.beams, [size]: { ...b, stock: parseInt(e.target.value) || 0 } } }))} className="w-20 border border-purple-200 rounded px-2 py-1 text-sm text-center font-bold" /></td>
                        <td className="px-3 py-2 text-right text-gray-500 text-xs">{totalRft.toLocaleString("en-IN")} ft</td>
                        <td className="px-3 py-2 text-center"><button onClick={() => { if (!confirm(`Delete ${size}ft beam?`)) return; setTI((ti) => { const next = { ...ti.beams }; delete next[size]; return { ...ti, beams: next }; }); }} className="text-red-400 hover:text-red-600 text-xs">✕</button></td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="mt-3 bg-purple-50 border border-purple-100 rounded-xl p-3 flex items-center justify-between">
              <div><p className="text-sm font-bold text-purple-800">💰 {matLabel} beam rental rate</p><p className="text-xs text-purple-600">Single rate for all beam sizes (₹/RFT × length).</p></div>
              <div className="flex items-center gap-2"><span className="text-sm text-gray-700">₹</span><input type="number" min="0" value={TI.rates?.beamRftRate || 0} onChange={(e) => setTI((ti) => ({ ...ti, rates: { ...ti.rates, beamRftRate: parseFloat(e.target.value) || 0 } }))} className="w-24 border border-purple-300 rounded-lg px-3 py-1.5 text-sm font-bold text-purple-700 text-center" /><span className="text-sm text-gray-700">/ RFT</span></div>
            </div>
          </div>

          {/* Batta — shared across materials, edited only in Iron */}
          {isIron && (
          <div className="bg-white border border-amber-100 rounded-2xl p-5">
            <div className="mb-3"><p className="font-bold text-amber-800">🧵 Batta (truss wrap) <span className="text-[10px] font-normal text-gray-400">· shared across Pole / Iron / Aluminium</span></p><p className="text-xs text-gray-500">Bulk RFT pool — ops cuts to needed length. Wraps every pillar + beam.</p></div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            <p className="font-bold text-gray-800 mb-1">⚙️ {matLabel} engineering settings</p>
            <p className="text-xs text-gray-500 mb-4">Layer 1 topology rules. Change only if engineering practice changes.</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div><label className="text-xs text-gray-600 font-medium">Pillar width (ft)</label><input type="number" step="0.05" min="0" value={TI.settings?.pillarWidthFt || 0.75} onChange={(e) => setTI((ti) => ({ ...ti, settings: { ...ti.settings, pillarWidthFt: parseFloat(e.target.value) || 0.75 } }))} className="mt-1 w-full border rounded-lg px-3 py-2 text-sm font-bold text-gray-700 text-center" /></div>
              <div><label className="text-xs text-gray-600 font-medium">Max span (ft)</label><input type="number" min="1" value={TI.settings?.maxSpanFt || 30} onChange={(e) => setTI((ti) => ({ ...ti, settings: { ...ti.settings, maxSpanFt: parseInt(e.target.value) || 30 } }))} className="mt-1 w-full border rounded-lg px-3 py-2 text-sm font-bold text-gray-700 text-center" /></div>
              <div><label className="text-xs text-gray-600 font-medium">Default back depth (ft)</label><input type="number" min="1" value={TI.settings?.defaultBackDepthFt || 4} onChange={(e) => setTI((ti) => ({ ...ti, settings: { ...ti.settings, defaultBackDepthFt: parseInt(e.target.value) || 4 } }))} className="mt-1 w-full border rounded-lg px-3 py-2 text-sm font-bold text-gray-700 text-center" /></div>
              <div><label className="text-xs text-gray-600 font-medium">Default truss type</label>
                <select value={TI.settings?.untaggedFallback || "half_box"} onChange={(e) => setTI((ti) => ({ ...ti, settings: { ...ti.settings, untaggedFallback: e.target.value } }))} className="mt-1 w-full border rounded-lg px-3 py-2 text-sm font-bold text-gray-700">
