...
export default function AdminTab({ users, setUsers, addUser, vendors, setVendors, functions, settings, setSettings, supervisors, setSupervisors, studio, inventory, trussInv, rateCardCategories, onUpdateSubcatFactor, onUpdateSubcatCostPercent, onAddSubcat, onRenameSubcat, onUpdateSubcatCategory, rcItems, rcCats, onSaveRateCardItems, onSaveRateCardCats }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {sub === "settings" && <AdminSettingsTab settings={settings} setSettings={setSettings} supervisors={supervisors} setSupervisors={setSupervisors} studio={studio} inventory={inventory} trussInv={trussInv} rateCardCategories={rateCardCategories} onUpdateSubcatFactor={onUpdateSubcatFactor} onUpdateSubcatCostPercent={onUpdateSubcatCostPercent} onAddSubcat={onAddSubcat} onRenameSubcat={onRenameSubcat} onUpdateSubcatCategory={onUpdateSubcatCategory} rcItems={rcItems} rcCats={rcCats} onSaveRateCardItems={onSaveRateCardItems} onSaveRateCardCats={onSaveRateCardCats} />}
    </div>
  );
}
