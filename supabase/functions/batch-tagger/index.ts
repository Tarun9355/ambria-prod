...
11. STRUCTURES vs TRUSS DIMS — these are TWO SEPARATE things and you must fill BOTH when relevant, never one instead of the other. The truss/platform/masking dims capture ONLY the plain overhead scaffold/base rig (Box Truss or Single U Truss), regardless of what it's made of or shaped like. SEPARATELY — and in ADDITION — if the structure itself (arch, panel, wall, jali, backdrop frame) has a distinct MATERIAL (Wooden or Wrought Iron) and shape, you MUST ALSO add ONE element for it using the exact sub-category name for that MATERIAL + DEPTH (2D flat / 3D with visible depth) + SHAPE (Arch/Panel) combination from the sub-category vocabulary below (e.g. a circular iron-jali arch structure → "Wrought Iron 2D Arch" or "Wrought Iron 3D Arch" depending on depth). Do NOT skip this element just because you already tagged the plain truss/platform.
